...
class Score():
    ...
    def animate(self, trickScore=None):
        if trickScore != None:
            self.max += trickScore
            play_sound('Assets/score_add.wav')
            self.tricks += 1
        if self.score < self.max:
            self.score += 50
            if self.score % 4999 == 0:
                play_sound('Assets/score_add.wav')
        if self.max != 0:
            if game.framecount-game.player.initJump <= ((5*60)+40)+(60*6):
...
class Game:
    ...
    def mainMenu(self):
        ...
        while True:
            ...
            drawText('BROKEN BONEZ', 230, 1920//2, 130, (255,255,0), 'Assets\\ipaexg.ttf')
            drawText('PRESS SPACE TO START', 100, 1920//2, 1080//2)
            for button in buttons:
                button.draw()
    ...
    def play(self, lives, score=0, timeLeft=60):
        ...
        while True:
            ...
            if self.player.rect.colliderect(self.obstacle.rect):
                if self.player.onRamp == False:
                    self.player.onRamp = True
                    self.player.image = pygame.transform.rotate(self.player.image, 23)
            elif self.player.onRamp: # Update this flag to false
                self.player.onRamp = False
                self.player.initJump = self.framecount

            # in air
            elif self.player.isJumping:
                # check multiplier
                if (self.framecount-self.player.initJump) % 60*6 == 0 and trick != False:
                    if multiplier < 5:
                        multiplier += 1
                else: multiplier = 1
            ...
            if not self.player.isJumping or crash != False:
                ...
                if len(self.animations) == 0:
                    ...
                    #pygame.draw.rect(self.screen, black, (self.player.x-25,220,50,50))
            ...
            scoreDisplay.animate(trickScore)
            trickScore = None
            if self.player.trickcount > 0:
        ...
        while True:
            ...
            if self.player.rect.colliderect(self.obstacle.rect):
                if self.player.onRamp == False:
                    self.player.onRamp = True
                    self.player.image = pygame.transform.rotate(self.player.image, 23)
            elif self.player.onRamp:
                self.player.onRamp = False
                self.player.initJump = self.framecount
                self.player.image = get_image('Assets\\bbm.png')
...
trick1animation = ['Assets\\Animations\\trick1.1.png',
                   'Assets\\Animations\\trick1.2.png',
                   'Assets\\bbm.png']

trick2animation = ['Assets\\Animations\\trick2.1.png',
                   'Assets\\Animations\\trick2.2.png',
                   'Assets\\Animations\\trick2.3.png',
                   'Assets\\bbm.png']

trick3animation = ['Assets\\Animations\\trick3.1.png',
                   'Assets\\Animations\\trick3.2.png',
                   'Assets\\Animations\\trick3.3.png',
                   'Assets\\Animations\\trick3.4.png',
                   'Assets\\Animations\\trick3.5.png',
                   'Assets\\Animations\\trick3.6.png',
                   'Assets\\Animations\\trick3.7.png',
                   'Assets\\bbm.png']
